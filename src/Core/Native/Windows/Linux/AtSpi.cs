...
namespace WatiN.Core.Native.Windows.Linux
{
    internal class AtSpi
    {
        ...
        private static AtSpi _instance = null;
        private static object _lock = new object();
        private static List<IntPtr> _referencedObjectCache = new List<IntPtr>();
        #endregion
        ...
        /// <summary>
        /// Destructor (required to properly dereference and free unmanaged resources).
        /// </summary>
        ~AtSpi()
        {
            //Dereference all of the Accessible objects in the cache
            if (_referencedObjectCache.Count > 0)
            {
                Console.WriteLine(string.Format("Found {0} referenced AT-SPI objects. Cleaning up references", _referencedObjectCache.Count.ToString()));
                foreach(IntPtr referencedObjectPointer in _referencedObjectCache)
                {
            ...
            int result = SPI_exit();
            if (result != 0)
                Console.WriteLine(string.Format("{0} leaks occurred in AT-SPI code", result.ToString()));
        }
        #endregion
        ...
        {
            if (_referencedObjectCache.Contains(obj))
            {
                _referencedObjectCache.Remove(obj);
        ...
        internal IList<IntPtr> FindChildrenWithRole(IntPtr parent, AccessibleRole desiredRole, bool visibleOnly, bool recursive)
        {
            ...
            for (int i = 0; i < childCount; i++)
            {
                ...
                IList<AccessibleState> stateList = GetStateList(childPointer);
                ...
                {
                    _referencedObjectCache.Add(childPointer);
                    childList.Add(childPointer);
                }

                //Call the function recursively to find all children at any depth.
                if (recursive)
                    childList.AddRange(FindChildrenWithRole(childPointer, desiredRole, visibleOnly, recursive));
        ...
        /// <param name="windowRect">A System.Drawing.Rectangle representing the window on the screen.</param>
        ...
        /// <returns>A System.IntPtr value that points to the Accessible object representing the top-level window.</returns>
        internal IntPtr MatchTopLevelWindowForApplication(string applicationName, Rectangle windowRect, WindowManagerFrameExtents frameExtents)
        {
            ...
            IList<IntPtr> topLevelWindows = GetTopLevelWindowsForApplication(applicationName);
            foreach(IntPtr candidateWindow in topLevelWindows)
            {
                if (Accessible_isComponent(candidateWindow))
                {
                    ...
                    {
                        _referencedObjectCache.Add(candidateWindow);
        ...
        private IList<IntPtr> GetTopLevelWindowsForApplication(string applicationName)
        {
	        ...
	        IntPtr application = GetApplication(applicationName);
